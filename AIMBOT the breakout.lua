...
local function addSelector(configKey, labelText, options, default)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        -- Khi đổi chế độ ESP, reset lại toàn bộ drawing và biến theo dõi
        if configKey == "espMode" then
            for _, drawings in pairs(espDrawings) do
                for _, d in pairs(drawings) do d:Remove() end
            end
            espDrawings = {}
            lastEspMode = newVal  -- Cập nhật để không xóa tiếp trong refreshESP
        end
...
addNote("RECOMMEND TURN OFF FOR BEST AIM ")
...
local function updateESPForZombie(zombie, drawings)
    ...
    if drawings["Outline"] then
        ...
        outline.Visible = config.espOutline  -- Sửa: tôn trọng config
    end
